...
class TagByPeriodeTable(Base):
    __tablename__ = 'tagByPeriodeTable'
    __table_args__ = {'extend_existing': True}
...
class UniqueTagNFrequencyTable(Base):
    __tablename__ = 'uniqueTagNFrequencyTable'
    __table_args__ = {'extend_existing': True}
...
class MonthlyTagTable(Base):
    __tablename__ = 'monthlyTagTable'
    __table_args__ = {'extend_existing': True}

    id = Column(Integer, primary_key=True)
    periodeM = Column(String(25))
    article_archives = Column(PickleType)
...
UniqueTagNFrequencyTable.__table__.create(bind=engine, checkfirst=True)
MonthlyTagTable.__table__.create(bind=engine, checkfirst=True)